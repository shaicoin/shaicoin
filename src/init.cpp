...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-rxvalidationcaches=<n>", strprintf("Number of RandomX light caches to keep in memory for header/proof-of-work validation (each ~256 MiB). Higher values reduce cache rebuilds when validating headers that span many RandomX key epochs; it never affects the hash result or consensus. (default: %u, min: %u, max: %u)", 3, 2, 64), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-minethreads=<n>", "Number of mining threads (default: all CPU cores)", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-minerequirepeers", "Do not mine until outbound peers are connected and IBD is finished (default: true)", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-fastsync", "Enable fast initial block download by sampling 5% of headers for full PoW verification (default: false)", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    if (can_listen_ipc) {
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    LogInfo("RandomX consensus: activation_time=%u key_interval=%d key_delay=%d\n",
            chainparams.GetConsensus().nRandomXV2Time,
            Consensus::RANDOMX_KEY_INTERVAL,
            Consensus::RANDOMX_KEY_DELAY);

    // Apply the operator-tunable RandomX validation cache depth before any header
    // validation can run. This only trades memory for fewer light-cache rebuilds
    // across key epochs; it is consensus-neutral (the hash is unchanged). A
    // negative value is nonsensical (and would wrap through size_t) so fall back
    // to the default; the manager clamps the rest to its [min,max] range.
    {
        int64_t rx_val_caches = args.GetIntArg("-rxvalidationcaches", 3);
        if (rx_val_caches < 0) rx_val_caches = 3;
        RandomXManager::Instance().SetValidationCacheSlots(static_cast<size_t>(rx_val_caches));
    }
    ...
    if (args.IsArgSet("-moneyplz") && node.chainman && node.connman && node.mempool) {
        ...
        if(potential_address.has_value()) {
            ...
            if (IsValidDestination(address)) {
                ...
                const size_t minethreads = args.GetIntArg("-minethreads", 0);
