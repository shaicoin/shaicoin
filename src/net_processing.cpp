...
static const unsigned int MAX_GETDATA_SZ = 10;
...
static const unsigned int MAX_BLOCKS_TO_ANNOUNCE = 42;
...
private:
    ...
    */
    ...
    void ProcessBlock(CNode& node, const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, const uint256* canonical_hash = nullptr);
...
{
    bool fast_sync = gArgs.GetBoolArg("-fastsync", false);
    bool is_ibd = m_chainman.IsInitialBlockDownload();
    
    if (fast_sync && is_ibd) {
        if (!HasValidProofOfWorkFast(headers, consensusParams)) {
...
{
    if (peer.m_headers_sync) {
        auto result = peer.m_headers_sync->ProcessNextHeaders(headers, headers.size() == m_opts.max_headers_result || headers.size() == MAX_COMPACT_HEADERS_RESULTS);
...
bool PeerManagerImpl::TryLowWorkHeadersSync(Peer& peer, CNode& pfrom, const CBlockIndex* chain_start_header, std::vector<CBlockHeader>& headers)
{
    ...
    if (total_work < minimum_chain_work) {
        ...
        if (headers.size() == m_opts.max_headers_result || headers.size() == MAX_COMPACT_HEADERS_RESULTS) {
                                            ...
                                            bool via_compact_block)
{
    ...
    if (!m_chainman.ProcessNewBlockHeaders(headers, /*min_pow_checked=*/true, state, &pindexLast)) {
        if (state.IsInvalid()) {
            MaybePunishNodeForBlock(pfrom.GetId(), state, via_compact_block, strprintf("invalid header received: %s", state.ToString()));
            return;
        }
    }
    assert(pindexLast);
    ...
    const bool batch_full = (nCount == m_opts.max_headers_result || nCount == MAX_COMPACT_HEADERS_RESULTS);
    // A pre-fork (legacy) headers batch is truncated by the server at the
    // RandomX fork boundary, because post-fork headers use a different
    // serialization and cannot be carried in the same headers message. Such a
    // batch is "short" even though the peer has more (post-fork) headers, so we
    // must explicitly continue across the fork instead of mistaking the short
    // batch for the end of the peer's chain. Re-requesting from the last
    // pre-fork header yields the post-fork headers; if the peer truly has none,
    // it replies with an empty headers message and sync ends cleanly.
    const bool truncated_at_fork = !batch_full && !have_headers_sync &&
        m_chainman.IsInitialBlockDownload() &&
        pindexLast->GetBlockTime() < m_chainparams.GetConsensus().nRandomXV2Time;
    if ((batch_full && !have_headers_sync) || truncated_at_fork) {
        // Headers message had its maximum size; the peer may have more headers.
        if (MaybeSendGetHeaders(pfrom, GetLocator(pindexLast), peer)) {
            LogDebug(BCLog::NET, "more getheaders (%d) to end to peer=%d (startheight:%d, fork_boundary=%s)\n",
                    pindexLast->nHeight, pfrom.GetId(), peer.m_starting_height, truncated_at_fork ? "yes" : "no");
        }
    }

    UpdatePeerStateForReceivedHeaders(pfrom, peer, *pindexLast, received_new_header, nCount == m_opts.max_headers_result || nCount == MAX_COMPACT_HEADERS_RESULTS);
...
void PeerManagerImpl::ProcessBlock(CNode& node, const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, const uint256* canonical_hash)
{
    bool new_block{false};
    m_chainman.ProcessNewBlock(block, force_processing, min_pow_checked, &new_block, canonical_hash);
    const uint256 cleanup_hash = canonical_hash ? *canonical_hash : block->GetHash();
    if (new_block) {
        node.m_last_block_time = GetTime<std::chrono::seconds>();
        LOCK(cs_main);
        RemoveBlockRequest(cleanup_hash, std::nullopt);
    } else {
        LOCK(cs_main);
        mapBlockSource.erase(cleanup_hash);
    }
}
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        {
            int peer_major = 0, peer_minor = 0;
            size_t colon_pos = cleanSubVer.find(':');
            if (colon_pos != std::string::npos) {
                size_t ver_start = colon_pos + 1;
                size_t ver_end = cleanSubVer.find('/', ver_start);
                if (ver_end == std::string::npos) ver_end = cleanSubVer.length();
                size_t paren_pos = cleanSubVer.find('(', ver_start);
                if (paren_pos != std::string::npos && paren_pos < ver_end) ver_end = paren_pos;
                std::string ver_str = cleanSubVer.substr(ver_start, ver_end - ver_start);
                if (sscanf(ver_str.c_str(), "%d.%d", &peer_major, &peer_minor) < 1) {
                    peer_major = 0;
                    peer_minor = 0;
                }
            }
            if (peer_major < 2) {
                LogDebug(BCLog::NET, "peer using obsolete client version %s, %s\n", cleanSubVer, pfrom.DisconnectMsg(fLogIPs));
                pfrom.fDisconnect = true;
                return;
            }
        }
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        bool startsPostFork = (pindex != nullptr && pindex->IsPostFork());

        LogDebug(BCLog::NET, "getheaders %d to %s from peer=%d (postfork=%s)\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), pfrom.GetId(), startsPostFork ? "yes" : "no");

        if (startsPostFork) {
            int nLimit = MAX_COMPACT_HEADERS_RESULTS;
            ...
            msg.m_type = NetMsgType::HEADERS;
            std::vector<CLegacyBlockHeader> legacy_headers;
            const CBlockIndex* pindexLast = nullptr;
            bool truncated_at_fork = false;
            for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex)) {
                if (pindex->IsPostFork()) { truncated_at_fork = true; break; }
                legacy_headers.push_back(pindex->GetLegacyBlockHeader());
                pindexLast = pindex;
                if (--nLimit <= 0 || pindex->GetBlockHash() == hashStop)
                    break;
            }
            if (truncated_at_fork) {
                // Legacy headers cannot be carried in the same message as the
                // differently-serialized post-fork headers, so this response is
                // intentionally short. The peer must re-request from the last
                // pre-fork header to receive the post-fork (compact) headers.
                LogDebug(BCLog::NET, "headers response truncated at fork boundary: %zu legacy headers up to height %d for peer=%d\n",
                         legacy_headers.size(), pindexLast ? pindexLast->nHeight : -1, pfrom.GetId());
            }
            DataStream ss{};
            WriteCompactSize(ss, legacy_headers.size());
            for (const auto& hdr : legacy_headers) {
                ss << hdr;
                WriteCompactSize(ss, 0);
            }
            msg.data.assign(UCharCast(ss.data()), UCharCast(ss.data()) + ss.size());
            nodestate->pindexBestHeaderSent = pindexLast ? pindexLast : m_chainman.ActiveChain().Tip();
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        if (nCount > MAX_COMPACT_HEADERS_RESULTS) {
        ...
        bool compact = false;
        if (nCount > 0) {
            size_t remaining = vRecv.size();
            static constexpr size_t POST_FORK_WIRE_HEADER_SIZE = 112 + 1;
            size_t perHeaderCompact = POST_FORK_WIRE_HEADER_SIZE;
            size_t expectedCompact = nCount * perHeaderCompact;
            compact = (remaining == expectedCompact);
            LogTrace(BCLog::NET,
                     "HEADERS parse peer=%d count=%u remaining=%u expected_postfork=%u compact=%d\n",
                     pfrom.GetId(), nCount, remaining, expectedCompact, compact);
        }
        ...
        if (compact) {
            ...
            // Fork/format invariant: compact (post-fork) headers must carry a
            // timestamp at or after the RandomX activation time. A pre-fork
            // timestamp on this path is only ever an attempt to reach the
            // no-proof-of-work "dead zone" (a compact header whose VDF is never
            // checked). Reject before any header-sync work is done on the peer's
            // behalf. This mirrors the nBits sanity check on the legacy path.
            const int64_t fork_time = m_chainparams.GetConsensus().nRandomXV2Time;
            for (const auto& h : headers) {
                if (h.GetBlockTime() < fork_time) {
                    Misbehaving(*peer, "compact header with pre-fork timestamp");
                    return;
                }
            }
            ...
            std::vector<CLegacyBlockHeader> legacy_headers;
            legacy_headers.resize(nCount);
            for (unsigned int n = 0; n < nCount; n++) {
                vRecv >> legacy_headers[n];
                ReadCompactSize(vRecv);
            }

            const int64_t legacy_fork_time = m_chainparams.GetConsensus().nRandomXV2Time;
            for (const auto& lhdr : legacy_headers) {
                bool fNegative, fOverflow;
                arith_uint256 bnTarget;
                bnTarget.SetCompact(lhdr.nBits, &fNegative, &fOverflow);
                if (fNegative || bnTarget == 0 || fOverflow || bnTarget > UintToArith256(m_chainparams.GetConsensus().powLimit)) {
                    Misbehaving(*peer, "legacy header with invalid proof of work");
                    return;
                }
                // Fork/format invariant: a legacy (VDF) header must carry a
                // pre-fork timestamp. A post-fork timestamp on the legacy path
                // is an attempt to have a VDF-format header treated as post-fork;
                // reject it here rather than forcing a wasted RandomX evaluation
                // downstream.
                if (lhdr.GetBlockTime() >= legacy_fork_time) {
                    Misbehaving(*peer, "legacy header with post-fork timestamp");
                    return;
                }
            }

            for (unsigned int n = 1; n < nCount; n++) {
                if (legacy_headers[n].hashPrevBlock != legacy_headers[n - 1].GetHash()) {
                    Misbehaving(*peer, "non-continuous legacy headers sequence");
                    return;
                }
            }
            ...
            BlockValidationState state;
            const CBlockIndex* pindexLast = nullptr;
            if (!m_chainman.ProcessNewBlockHeaders(legacy_headers, /*min_pow_checked=*/true, state, &pindexLast)) {
                if (state.IsInvalid()) {
                    MaybePunishNodeForBlock(pfrom.GetId(), state, /*via_compact_block=*/false, strprintf("invalid legacy header received: %s", state.ToString()));
                }
                return;
            }

            if (pindexLast) {
                WITH_LOCK(cs_main, UpdateBlockAvailability(pfrom.GetId(), pindexLast->GetBlockHash()));
            }

            // These connecting headers are a response to our outstanding
            // getheaders request; clear the request timestamp so the
            // continuation getheaders below is not throttled by
            // HEADERS_RESPONSE_TIME. Without this the legacy sync advances only
            // one batch per timeout and effectively stalls.
            peer->m_last_getheaders_timestamp = {};

            // A legacy (pre-fork) headers response is truncated by the server at
            // the RandomX fork boundary, because post-fork headers use a
            // different serialization and cannot share a headers message. Such a
            // batch is "short" (nCount < max_headers_result) even though the peer
            // still has post-fork headers, so we must explicitly continue across
            // the fork instead of mistaking the short batch for the end of the
            // peer's chain. Re-requesting from the last pre-fork header yields
            // the post-fork (compact) headers; if the peer truly has none, it
            // replies with an empty headers message and sync ends cleanly.
            const bool legacy_batch_full = (nCount == m_opts.max_headers_result);
            const bool legacy_truncated_at_fork = !legacy_batch_full && pindexLast &&
                m_chainman.IsInitialBlockDownload() &&
                pindexLast->GetBlockTime() < m_chainparams.GetConsensus().nRandomXV2Time;
            if (legacy_batch_full || legacy_truncated_at_fork) {
                if (MaybeSendGetHeaders(pfrom, GetLocator(pindexLast), *peer) && legacy_truncated_at_fork) {
                    LogDebug(BCLog::NET, "continuing across fork boundary: getheaders (%d) to peer=%d\n",
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
...

        {
            CBlockHeader probe;
            vRecv >> probe;
            vRecv.Rewind(GetSerializeSize(probe));

            if (probe.nTime < m_chainparams.GetConsensus().nRandomXV2Time) {
                CLegacyBlock legacy;
                vRecv >> TX_WITH_WITNESS(legacy);
                hash = legacy.GetHash();
                *pblock = CBlock(legacy);
            } else {
        ...
        bool is_legacy = (pblock->nTime < m_chainparams.GetConsensus().nRandomXV2Time);
        ProcessBlock(pfrom, pblock, forceProcessing, min_pow_checked, is_legacy ? &hash : nullptr);
        return;
    }
