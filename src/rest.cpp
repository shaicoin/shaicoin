...
static constexpr unsigned int MAX_REST_HEADERS_RESULTS = 40;
static constexpr unsigned int MAX_REST_COMPACT_HEADERS_RESULTS = 2000;
                         ...
                         const std::string& strURIPart)
{
    ...
    if (!parsed_count.has_value() || *parsed_count < 1 || *parsed_count > MAX_REST_COMPACT_HEADERS_RESULTS) {
        return RESTERR(req, HTTP_BAD_REQUEST, strprintf("Header count is invalid or out of acceptable range (1-%u): %s", MAX_REST_COMPACT_HEADERS_RESULTS, raw_count));
    }
