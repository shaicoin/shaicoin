...
uint256 CLegacyBlockHeader::GetHash() const
{
    if(nTime <= 1723869065) {
...
uint256 CLegacyBlockHeader::GetSHA256() const
{
    CLegacyBlockHeader no_vdf{};
...
std::string CLegacyBlock::ToString() const
{
    std::stringstream s;
    s << strprintf("CLegacyBlock(hash=%s, ver=0x%08x, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, nBits=%08x, nNonce=%u, vtx=%u)\n",
        GetHash().ToString(),
        nVersion,
