    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->hashExtCommitment = diskindex.hashExtCommitment;
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block, CBlockIndex*& best_header, const uint256* override_hash)
{
    AssertLockHeld(cs_main);

    const uint256& hash = override_hash ? *override_hash : block.GetHash();
...
CBlockIndex* BlockManager::AddToBlockIndex(const CLegacyBlockHeader& block, CBlockIndex*& best_header)
{
    AssertLockHeld(cs_main);
...
void BlockManager::UpdateBlockInfo(const CLegacyBlock& block, unsigned int nHeight, const FlatFilePos& pos)
{
    LOCK(cs_LastBlockFile);
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos) const
{
    ...
    try {
        CBlockHeader probe;
        filein >> probe;
        filein.seek(-static_cast<int64_t>(GetSerializeSize(probe)), SEEK_CUR);

        if (probe.nTime < GetConsensus().nRandomXV2Time) {
            CLegacyBlock legacyBlock;
            filein >> TX_WITH_WITNESS(legacyBlock);
            block = CBlock(legacyBlock);
        } else {
...
bool BlockManager::ReadLegacyBlock(CLegacyBlock& block, const FlatFilePos& pos) const
{
    block.SetNull();
...
bool BlockManager::ReadBlock(CBlock& block, const CBlockIndex& index) const
{
    ...
    if (index.IsPostFork()) {
        ...
        CLegacyBlock legacyBlock;
        if (!ReadLegacyBlock(legacyBlock, block_pos)) {
            return false;
        }
        if (legacyBlock.GetHash() != index.GetBlockHash()) {
            LogError("%s: GetHash() doesn't match index for %s at %s\n", __func__, index.ToString(), block_pos.ToString());
            return false;
        }
        block = CBlock(legacyBlock);
...
FlatFilePos BlockManager::WriteLegacyBlock(const CLegacyBlock& block, int nHeight)
{
    const unsigned int block_size{static_cast<unsigned int>(GetSerializeSize(TX_WITH_WITNESS(block)))};
    FlatFilePos pos{FindNextBlockPos(block_size + BLOCK_SERIALIZATION_HEADER_SIZE, nHeight, block.GetBlockTime())};
    if (pos.IsNull()) {
