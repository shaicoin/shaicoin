...
const char * const BITCOIN_CONF_FILENAME = "shai.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Shaicoin
    //   new: C:\Users\Username\AppData\Local\Shaicoin
    // macOS: ~/Library/Application Support/Shaicoin
    // Unix-like: ~/.shaicoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Shaicoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Shaicoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Shaicoin";
#else
    // Unix-like
    return pathRet / ".shaicoin";
#endif
#endif
}
